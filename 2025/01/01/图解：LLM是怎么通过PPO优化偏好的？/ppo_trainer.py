...
class PPOTrainer(BaseTrainer):
    ...
    def step(
        ...
        scores = torch.tensor(scores, device=self.current_device)   # (batch_size,)
        ...
        model_inputs = self.prepare_model_inputs(queries, responses)    # {"input_ids", ..., "attention_mask": ...} (batch_size, sequence_length)
        ...
        with torch.no_grad():
            all_logprobs, logits_or_none, values, masks = self.batched_forward_pass(
                self.model,
                queries,            # List[torch.Tensor (query_length,)]
                responses,          # List[torch.Tensor (response_length,)]
                model_inputs,       # {"input_ids", ..., "attention_mask": ...} (batch_size, sequence_length)
            ...
            for backward_batch_start in range(0, bs, self.config.backward_batch_size):
                ...
                for mini_batch_start in range(0, self.config.backward_batch_size, self.config.mini_batch_size):
                    ...
                    mini_batch_dict = {
                        ...
                        "masks": batch_dict["masks"][mini_batch_inds],              # (mini_batch_size, sequence_length - 1)
                        "logprobs": batch_dict["logprobs"][mini_batch_inds],        # (mini_batch_size, sequence_length - 1)
                        "values": batch_dict["values"][mini_batch_inds],            # (mini_batch_size, sequence_length - 1)
                        "returns": batch_dict["returns"][mini_batch_inds],          # (mini_batch_size, sequence_length - 1)
                        "advantages": batch_dict["advantages"][mini_batch_inds],    # (mini_batch_size, sequence_length - 1)
                    }
    ...
    def batched_forward_pass(
        ...
        for i in range(math.ceil(bs / fbs)):
            ...
            logits, _, values = model(**input_kwargs)   # (mini_batch_size, sequence_length, vocab_size), (mini_batch_size, sequence_length)

            if self.is_encoder_decoder:
                input_ids = input_kwargs["decoder_input_ids"]
                attention_mask = input_kwargs["decoder_attention_mask"]
            else:
                input_ids = input_kwargs["input_ids"]           # (mini_batch_size, sequence_length)
                attention_mask = input_kwargs["attention_mask"] # (mini_batch_size, sequence_length)

            logprobs = logprobs_from_logits(logits[:, :-1, :], input_ids[:, 1:])    # (mini_batch_size, sequence_length - 1)
            masks = torch.zeros_like(attention_mask)    # 记录 response token 位置
        ...
        return (
            torch.cat(all_logprobs),        # (batch_size, sequence_length - 1)
            torch.cat(all_logits)[:, :-1] 
                if return_logits else None, # (batch_size, sequence_length - 1)
            torch.cat(all_values)[:, :-1],  # (batch_size, sequence_length - 1)
            torch.cat(all_masks)[:, :-1],   # (batch_size, sequence_length - 1)
        )
    ...
    def compute_rewards(
        self,
        scores: torch.FloatTensor,      # (batch_size,)
        logprobs: torch.FloatTensor,    # (batch_size, sequence_length - 1)
        ref_logprobs: torch.FloatTensor,# (batch_size, sequence_length - 1)
        masks: torch.LongTensor,        # (batch_size, sequence_length - 1)
    ):
        """
        ...
        for score, logprob, ref_logprob, mask in zip(scores, logprobs, ref_logprobs, masks):
            # （1,) (sequence_length - 1,) (sequence_length - 1,) (sequence_length - 1,)

            # compute KL penalty (from difference in logprobs)
            kl = self._kl_penalty(logprob, ref_logprob) # (sequence_length - 1,)
            kls.append(kl)

            non_score_reward = -self.kl_ctl.value * kl  # (sequence_length - 1,)
            non_score_rewards.append(non_score_reward)
            
            # reward is preference model score + KL penalty
            reward = non_score_reward.clone()           # (sequence_length - 1,)
            last_non_masked_index = mask.nonzero()[-1]
            reward[last_non_masked_index] += score
            rewards.append(reward)

        return torch.stack(rewards), torch.stack(non_score_rewards), torch.stack(kls)   # (batch_size, sequence_length - 1)
    ...
    def compute_advantages(
        ...
        lastgaelam = 0
        advantages_reversed = []
        gen_len = rewards.shape[-1]     # sequence_length - 1
        ...
        for t in reversed(range(gen_len)):
            nextvalues = values[:, t + 1] if t < gen_len - 1 else 0.0               # (batch_size,)
            delta = rewards[:, t] + self.config.gamma * nextvalues - values[:, t]   # (batch_size,) a_t = r_t + \gamma V_{r+1} - V_t
            lastgaelam = delta + self.config.gamma * self.config.lam * lastgaelam   # (batch_size,) a_t \leftarrow a_t + \gamma \lambda a_{t+1}
            advantages_reversed.append(lastgaelam)
        advantages = torch.stack(advantages_reversed[::-1]).transpose(0, 1)         # (batch_size, sequence_length - 1)
